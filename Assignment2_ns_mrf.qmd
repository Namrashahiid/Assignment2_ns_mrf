...
We can see that there is a peak in age group, 12, 14 and 16 years.
What we have done in this graph is that we have divided the variable into four subcategories: not high-school (0-12 years), not trade collage (12-14 years) and collage+ (16-21 years).
...
What we see in this graph is that flatter areas which contains collage + education, but it seems like there are more in this graph who has higher education.
The lower the income, the lower the curves at the start.
We can also see that the longer and higher the education, thus the higher the income.
...
The categories we see here in this graph is very low, low, medium and high.
In this graph we can see how age is distributed in different age categories.
What we can conclude with here is that there are many categories under categories in this graph.
...
In this histogram graph what we see is the comparison between height and weight in the first one.
And height and afqt in the second one.
The second histogram looks more a like balanced, when we look at the bars.
